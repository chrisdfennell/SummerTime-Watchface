...
if ($Export) {
    ...
    $outputPath = Join-Path $PSScriptRoot "bin\Summertime.iq"
    & $monkeyc -e -f $junglePath -o $outputPath -y $keyPath
} else {
    Write-Host "Building for device: $Device..." -ForegroundColor Cyan
    $outputPath = Join-Path $PSScriptRoot "bin\Summertime.prg"
...
if ($Export) {
    Write-Host "Package Succeeded! Output: bin\Summertime.iq" -ForegroundColor Green
} else {
    Write-Host "Build Succeeded! Output: bin\Summertime.prg" -ForegroundColor Green
}
