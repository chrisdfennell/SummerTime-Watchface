...
Thanks for your interest in improving Summertime! This is a golden-hour beach themed Garmin Connect IQ watch face for the Fenix 8 and tactix 8, written in [Monkey C](https://developer.garmin.com/connect-iq/monkey-c/). Contributions of all kinds are welcome — bug reports, layout/styling improvements, new indicators, device support, art/font assets, and documentation.
...
- `source/SummertimeApp.mc` / `SummertimeView.mc` — the app + watch face (all rendering is procedural in `onUpdate`).
...
Summertime targets the **Fenix 8 and tactix 8** in both case sizes. Please verify your change on both before submitting:
...
- Match the existing style in `source/SummertimeView.mc`: 4-space indentation, explicit type annotations on method signatures, and `private var` for fields.
...
- Complication / XP palettes are the `C_*` constants at the top of the view; layout anchors are the percentage values in `onUpdate()`.
...
1. Fork the repo and create a topic branch off `main` (e.g. `feature/palm-wind` or `fix/battery-label-clip`).
...
```
feat: add palm leaf wind animation
